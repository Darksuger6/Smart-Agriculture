...
#define AUTH_INFO	"syw666"

#define DEVID		"1205733705"
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));//温度
	sprintf(text, "Tempreture:%d,", dht11_info.tempreture);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//湿度
	sprintf(text, "Humidity:%d,", dht11_info.humidity);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//土壤湿度
	sprintf(text, "Soil:%d,", adc_info.Soil);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//光照强度
	sprintf(text, "Light:%d,", adc_info.Light);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//土壤湿度阈值
	sprintf(text, "Soilvalue:%d,", Soilvalue);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//光照强度阈值
	sprintf(text, "Lightvalue:%d,", Lightvalue);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//补水客开关
	sprintf(text, "Soilkey:%d,", Soilkey);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//补光开关
	sprintf(text, "Lightkey:%d,", Lightkey);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));//补光开关
	sprintf(text, "Autoflag:%d", Autokey);
	strcat(buf, text);
	//memset(text, 0, sizeof(text));
...
//						printf("json pack into cjson error...");
				...
				Autoflag = cJSON_GetObjectItem(cjson,"Auto")->valueint;
				if(Autoflag == 1)	Autokey = 1;
				if(Autoflag == 0) Autokey = 0;
					
				Lightflag = cJSON_GetObjectItem(cjson,"LED")->valueint;
				if(Lightflag == 1)	Lightkey = 1;
				if(Lightflag == 0) Lightkey = 0;
				
				Soilflag = cJSON_GetObjectItem(cjson,"Water")->valueint;
				if(Soilflag == 1)	Soilkey = 1;
				if(Soilflag == 0) Soilkey = 0;
				
				
				Soil = cJSON_GetObjectItem(cjson,"Soil")->valueint;
				if(Soil <= 100) Soilvalue = Soil;
				Sun = cJSON_GetObjectItem(cjson,"Sun")->valueint;
				if(Sun  <= 100)	Lightvalue = Sun;
